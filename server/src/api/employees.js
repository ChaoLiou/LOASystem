  ...
  return {
    ...
    employees: employees
      ...
      .map(employee => {
        ...
        const compensatoryLTsStashed = employee.activatedLeaveTypes.filter(
          c.predicate.isCompensatoryLeaveTypeStashed
        );
...
function produceCompensatoryInfo(
  ...
  return {
    ...
    arrivedDate,
    totals: {
      days: consumes.days + amountOfCompensatoryLTStashed,
    ...
    leaveTypes: compensatoryLTs
      .filter(
        leaveType => !c.predicate.isLeaveTypeNoMoreLeaves(leaveType) ||
      ...
      .filter(leaveType => !c.predicate.isCompensatoryLeaveTypeStashed(leaveType))
      .map(leaveType => {
        return {
