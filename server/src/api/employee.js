          ...
          if (err) {
            ...
            updateRecord(employee, req.body).save(err => {
              if (err) {
                ...
                try {
                  notifyLeaveTaking(employee, req.body.records);
                } catch(ex) {
                  console.log(ex); 
                }
                res.send({
              ...
              if (
                ...
                employee.save(err => {
                  if (err) {
                    ...
                    try {
                      notifyLeaveSigning(employee, loginuser, signedRecord);
                    } catch(ex) {
                      console.log(ex);
                    }
                    res.send({
...
function notifyLeaveSigning(taker, signer, record) {
  e.sendLeaveSigningEmail({ taker, signer, record });
  l.sendLeaveSigning({ taker, signer, record });
}

function notifyLeaveTaking(taker, records) {
  collectSignersInfos(taker.signers, [], (err, signers) => {
    e.sendLeaveTakingEmail({ taker, signers, records })
    l.sendLeaveTaking({ taker, signers, records });
  });
}
